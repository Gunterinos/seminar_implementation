...
def compute_predicate_sequence(
    ...
    for e in range(n_iter):
        ...
        # record predicate history for visualization
        predicate_history.append(calculate_predicates(
            selected, a, n_features, scale, mu, vmin, vmax, mean, x0, columns
        ))

        # record qualities history for visualization
        quality_history.append(calculate_qualities(
            selected, a, x, mu, label, n_points
        ))
...
def calculate_predicates(selected, a, n_features, scale, mu, vmin, vmax, mean, x0, columns):
